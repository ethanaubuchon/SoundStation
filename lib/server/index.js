...
// Play a song given its id, update the model, and redirect back to home
expressApp.get('/play/:id', function(req, res) {
  musicLibrary.playOrQueueSong(req.params.id, updateSongs);
  res.redirect('/');
});

// Play given imediately (skipping queue)
expressApp.get('/playnow/:id', function(req, res) {
  musicLibrary.playNow(req.params.id, updateSongs);
  res.redirect('/');
});

// Stop whatever song it currently playing and set it to the first in queue and redirect back to home
expressApp.get('/stop', function(req, res) {
  musicLibrary.stopSong(updateSongs);
  res.redirect('/');
});

// Skip to the next song in the playlist if one is not available and redirect back to home
expressApp.get('/next', function(req, res){
  musicLibrary.nextSong(updateSongs);
  res.redirect('/');
});

// Removes a song from the queue its queueId, update the model, and redirect back to home
expressApp.get('/remove/:id', function(req, res) {
  musicLibrary.removeSongFromQueue(req.params.id,updateSongs);
  res.redirect('/');
});

// Moves a song up one position in the queue give the queueId, update the model, and redirect back to home
expressApp.get('/moveup/:id', function(req, res) {
  musicLibrary.moveUpInQueue(req.params.id,updateSongs);
  res.redirect('/');
});

// Moves a song down one position in the queue give the queueId, update the model, and redirect back to home
