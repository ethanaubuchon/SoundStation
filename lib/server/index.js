...
if (config['enableVotingOrder'])
  musicLibrary.enableVotingOrder();

function updateArtists(artist) {
  musicLibrary.getSongsByAttr('artist', artist['name'], function(results) {
    model.set('artist'+artist['id'], results);
  });
}

function updateAlbums(album) {
  musicLibrary.getSongsByAttr('album', album['name'], function(results) {
    model.set('album'+album['id'], results);
  });
}

function updateGenres(genre) {
  musicLibrary.getSongsByAttr('genre', genre['name'], function(results) {
...
function updateShuffle() {
  model.set('shuffle', musicLibrary.shuffleSet());
}
...
expressApp.get('/play/:id', function(req, res) {
  ...
  musicLibrary.playOrQueueSong(req.params.id, sessionID, updateSongs);
  res.redirect('/');
});
...
expressApp.get('/resume/:id', function(req, res) {
  if (isAdmin(req))
    musicLibrary.playSong(req.params.id, updateSongs);
  res.redirect('/');
});
...
expressApp.get('/prev', function(req,res){
  if (isAdmin(req))
    musicLibrary.previousSong(updateSongs);
  res.redirect('/');
});
...
expressApp.get('/stopShuffle', function(req, res) {
  if (isAdmin(req))
    musicLibrary.stopShuffle(updateShuffle);
  res.redirect('/');
});
...
expressApp.get('/shuffle', function(req, res) {
  if (isAdmin(req))
    musicLibrary.shuffle(updateShuffle);
  res.redirect('/');
});
...
expressApp.get('/vote/:id', function(req, res) {
  var sessionID = req.getModel()['req']['sessionID'];
  musicLibrary.voteForSongInQueue(req.params.id, sessionID, updateSongs);
  res.redirect('/');
});
