...
musicDirs.forEach(function(directory) {
  musicLibrary.addToLibrarySync(directory, updateSongs);
});
...
expressApp.get('/addAll/:type(artist|album|genre)/:id', function(req, res) {
  ...
  if (type==="artist") array = musicLibrary.getArtists();
  else if (type==="genre") array = musicLibrary.getGenres();
  else if (type ==="album") array = musicLibrary.getAlbums();
  else return;
