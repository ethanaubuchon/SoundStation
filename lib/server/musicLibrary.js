...
// Uses callback() each time all songs in a directory have been parsed
exports.addToLibrarySync = function(dir, callback) {
  ...
  var updateOnLastSong = function() {
    songsCovered++;
    if (songsCovered == songPaths.length)
      callback();
  };
  ...
  for (i = 0; i < songPaths.length; i++) {
      addSong(songPaths[i], updateOnLastSong);
  }
