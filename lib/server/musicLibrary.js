...
// Can't simply use array indexes as songs/artists may be removed
var insertId = 1;
  ...
  async.each(list, function(file, callback) {
    ...
    // If it's an MP3, get its meta-data
      ...
      parser.on('done', function (err) {
        ...
        artistsHash[songInfo['artist']] = 1;
        albumsHash[songInfo['album']] = 1;
        genresHash[songInfo['genre']] = 1;
        ...
        if (songsCovered == numSongs) {
          // Update Artists
          var artistKeys = Object.keys(artistsHash);
          for (var i = 0, len = artistKeys.length; i < len; i++) {
            var artistInfo = { 'name': artistKeys[i], 'id': artistInsertId++ };
            artists.push(artistInfo);
          }

          // Update Albums
          var albumsKeys = Object.keys(albumsHash);
          for (var j = 0, len = albumsKeys.length; j < len; j++) {
            var albumInfo = { 'name': albumsKeys[j], 'id': albumInsertId++ };
            albums.push(albumInfo);
          }

          // Update Genres
          var genresKeys = Object.keys(genresHash);
          for (var k = 0, len = genresKeys.length; k < len; k++) {
            var genreInfo = { 'name': genresKeys[k], 'id': genreInsertId++ };
