...
app.get('/albums/', function(page, model) {
  model.subscribe('albums', 'currentSong', 'queue', 'playing', function() {
    page.render('albums', {
      'isAdmin': model["session"]["isAdmin"],
      'albumPage': 'active'
    });
...
app.get('/album/:id', function (page, model, params) {
  ...
  model.subscribe(albumPath, 'currentSong', 'queue', 'playing', function(err, artist) {
...
app.get('/genres/', function(page, model) {
  model.subscribe('genres', 'currentSong', 'queue', 'playing', function() {
    page.render('genres', {
      'isAdmin': model["session"]["isAdmin"],
      'genrePage': 'active'
    });
...
app.get('/genre/:id', function (page, model, params) {
  ...
  model.subscribe(genrePath, 'currentSong', 'queue', 'playing', function(err, artist) {
    // TODO: Use ref for albumPath
    page.render('songs', {
      'isAdmin': model["session"]["isAdmin"],
      'genrePage': 'active',
